...
function umberto_theme_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = UMBERTO_STORAGE['co_add_params']!=undefined ? UMBERTO_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#umberto-customizer-styles-'+option).length > 0 ? jQuery('#umberto-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="umberto-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
