...
( function( $ ) {
	...
	if (!umberto_previewer_vars['need_refresh']) {
		var $style = $('#umberto-customizer-css');
		// Prepare inline styles in the preview window
		if ( $style.length == 0 ) {
			$style = $('head').append( '<'+'style type="text/css" id="umberto-customizer-css"></'+'style'+'>' ).find('#umberto-customizer-css');
		}
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-preview', function( ) {
			"use strict";
			umberto_set_cookie('umberto_compile_less', 1);
		} );
